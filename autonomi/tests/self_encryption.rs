...
fn test_basic_encryption() {
    ...
    assert!(data_map_chunk.size() <= *MAX_CHUNK_SIZE, "Data map chunk should be within size limits");
}
...
fn test_chunk_size_boundary() {
    ...
    let result = encrypt(data);
    assert!(result.is_ok(), "Chunk size boundary encryption should succeed");
...
fn test_large_data_encryption() {
    ...
    for chunk in chunks {
        assert!(chunk.size() <= *MAX_CHUNK_SIZE, "Each chunk should be within size limits");
    }
}
...
fn test_multi_level_data_maps() {
    ...
    let result = encrypt(data);
    assert!(result.is_ok(), "Multi-level data map encryption should succeed");
